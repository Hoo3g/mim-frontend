...
export class PostService {
    ...
    getPosts(): Observable<Post[]> {
        return this.http.get<ApiResponse<ApiPostModel[]>>(API_ENDPOINTS.RECRUITMENT.LIST).pipe(
            map((response) => this.unwrapList(response).map((item) => this.toPostModel(item)))
        );
    }

    getMyPosts(_authorId: string): Observable<Post[]> {
    ...
    saveMyPost(payload: PostEditorPayload, _user: AuthUser, postId?: string): Observable<Post> {
    ...
    private toPostModel(item: ApiPostModel): Post {
        ...
        return {
            ...
            createdAt: this.toDate(item.createdAt, now),
            updatedAt: this.toDate(item.updatedAt, now)
        };
    }

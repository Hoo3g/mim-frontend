...
export class RecruitmentCategoryService {
    ...
    getActiveCategories(): Observable<ResearchCategory[]> {
        const cacheKey = 'active-recruitment-categories';
        const cachedCategories$ = this.categoriesCache.get(cacheKey);
        if (cachedCategories$) {
            return cachedCategories$;
        }

        const request$ = this.http.get<ApiResponse<ResearchCategory[]>>(API_ENDPOINTS.RECRUITMENT.CATEGORIES).pipe(
